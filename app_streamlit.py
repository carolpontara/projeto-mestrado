...
# ---------------------------------------------------------------------------
# SSRF protection – only allow requests to localhost / 127.0.0.1
# ---------------------------------------------------------------------------
...
def _is_safe_url(url: str) -> bool:
    """Return True only if the URL targets an allowed host."""
    try:
...
if st.button("Testar conexão"):
    if not _is_safe_url(api_url):
        st.error("URL não permitida. Use apenas localhost ou 127.0.0.1.")
    else:
        try:
            root = base_url_from_analyze(api_url)
            r = requests.get(root, timeout=5)
            st.success(f"Conectado em {root} (status {r.status_code})")
